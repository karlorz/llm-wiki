...
for dir in "$SKILLS_DIR"/wiki-* "$SKILLS_DIR"/proj-* "$SKILLS_DIR"/using-skillwiki; do
  name=$(basename "$dir")
...
ACTUAL_COUNT=$(ls -d "$SKILLS_DIR"/wiki-* "$SKILLS_DIR"/proj-* "$SKILLS_DIR"/using-skillwiki 2>/dev/null | wc -l | tr -d ' ')
