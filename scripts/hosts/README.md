...
3. Hosts with `maintenance.skillwiki_satellite.enabled=true` in `fleet.yaml` must keep `VAULT_PATH` equal to `maintenance.skillwiki_satellite.vault_path`. The sg02 satellite Git writer is retired (vault ADR `2026-09-17-research-mcp-on-sg01`). The schema-valid operator apply form is to **omit** `hosts.sg02.maintenance` (`jobs: []` fails `skillwiki fleet validate` with minItems 1). Until that PATH_DENIED YAML is applied, do not treat the stale enabled block as authority to run the timers, and do not add a git-mutating satellite on sg01.
