...
1. **Determine scope.** Ask the user once if ambiguous: vault | current project | project+concepts.
   ...
   - Setup, MCP, doctor, plugin-drift, or "open work" questions are not typed-knowledge searches. Call `wiki_context` (active_work) or `skillwiki query "<text>" --scope work` / `--scope all`. Default typed query never ranks `projects/*/work`.
