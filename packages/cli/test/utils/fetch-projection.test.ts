...
describe("resolveConfiguredFetchProjection", () => {
  it("returns the configured absolute projection path", () => {
    const projection = join(tmpdir(), "wiki-fetch");
    const home = makeHome(`vault_sync.fetch_projection=${projection}\n`);
    expect(resolveConfiguredFetchProjection(home)).toBe(projection);
  });
