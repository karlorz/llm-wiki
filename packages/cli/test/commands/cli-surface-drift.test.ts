...
describe("cli-surface drift detection", () => {
  it("surface includes all known top-level commands", () => {
    ...
    const knownCommands = [
      ...
      "lint", "config", "health", "doctor", "connect", "status", "copy-status", "archive", "remove", "drift", "dedup",
  ...
  it("surface includes all known subcommands", () => {
    ...
    const subcommandGroups = [
      ...
      { parent: "sources", subs: ["pending", "skipped", "disposition", "compile", "review", "reviews", "dispose"] },
      { parent: "vectors", subs: ["rebuild", "status", "reindex-page", "prune-page"] },
