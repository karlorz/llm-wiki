...
describe("skillwiki-maintenance CLI entrypoint detection", () => {
  it("treats an npm bin symlink as a direct CLI invocation", () => {
    const root = mkdtempSync(join(tmpdir(), "skillwiki-maintenance-cli-entry-"));
    const realDir = join(root, "packages", "skillwiki-maintenance", "dist");
    const binDir = join(root, "node_modules", ".bin");
    mkdirSync(realDir, { recursive: true });
    mkdirSync(binDir, { recursive: true });

    const realEntrypoint = join(realDir, "cli.js");
    const symlinkEntrypoint = join(binDir, "skillwiki-maintenance");
