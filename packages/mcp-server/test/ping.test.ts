...
import { mcpPingBeforeInitializeError } from "../src/mcp-initialize.js";
...
describe("MCP ping before initialize fail-closed", () => {
  it("rejects a JSON-RPC batch that pings before initialize", () => {
    const err = mcpPingBeforeInitializeError([
  ...
  it("does not intercept a standalone ping or a batch that initializes first", () => {
    expect(mcpPingBeforeInitializeError({
      jsonrpc: "2.0",
      id: 1,
      method: "ping",
      params: {},
...
describe("MCP ping HTTP before initialize fail-closed", () => {
  ...
  it("HTTP JSON-RPC batch ping before initialize returns no result and no writer_id", async () => {
    ...
    try {
      const res = await fetch(`http://127.0.0.1:${ctx.port}/mcp`, {
        ...
        body: JSON.stringify([
          ...
          { jsonrpc: "2.0", id: 72, method: "initialize", params: supportedInitialize },
        ]),
      });
      ...
      expect(body.error?.message).toBe("ping before initialize");
