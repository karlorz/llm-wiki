...
import { mcpRootsListBeforeInitializeError } from "../src/mcp-initialize.js";
...
describe("MCP roots/list before initialize fail-closed", () => {
  it("rejects a JSON-RPC batch that lists roots before initialize", () => {
    const err = mcpRootsListBeforeInitializeError([
  ...
  it("does not intercept a standalone roots/list or a batch that initializes first", () => {
    expect(mcpRootsListBeforeInitializeError({
      jsonrpc: "2.0",
      id: 1,
      method: "roots/list",
      params: {},
...
describe("MCP roots/list HTTP before initialize fail-closed", () => {
  ...
  it("HTTP JSON-RPC batch roots/list before initialize returns no result and no writer_id", async () => {
    ...
    try {
      const res = await fetch(`http://127.0.0.1:${ctx.port}/mcp`, {
        ...
        body: JSON.stringify([
          { jsonrpc: "2.0", id: 121, method: "roots/list", params: {} },
          { jsonrpc: "2.0", id: 122, method: "initialize", params: supportedInitialize },
        ]),
      });
      ...
      expect(body.error?.message).toBe("roots/list before initialize");
