...
This repo ships the `skillwiki` CLI and 19 prompt-only SKILL.md files.
...
- **`e2e-plugin.sh`** — verifies the Claude Code plugin channel on sg01: version, 19 SKILL.md files, skill discovery via claude, and CLI commands through the plugin path (27 assertions).
...
- 19 SKILL.md files in `packages/skills/`
- 41 source files in `packages/cli/src/commands/`
- 65 test files in cli (42 commands, 15 utils, 4 parsers, 2 integration, 1 skills, 1 smoke) + 9 shared
- 801 tests passing
