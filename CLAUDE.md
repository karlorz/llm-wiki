...
- Local dev build: `npm run -w packages/cli build` (output in `packages/cli/dist/`). Run with `node packages/cli/dist/cli.js <command>`. (An experimental `skillwiki mcp` / `skillwiki-mcp` stdio server exists under `packages/cli/src/mcp/` but is **hidden and unsupported** — not declared in plugin manifests or user docs. Revisit when the brain moves to a remote centralized wiki.)
...
- Antigravity/`agy` root plugin manifests: `plugin.json` (local validation/path installs) and `.claude-plugin/plugin.json` (GitHub URL installs). Direct install is `agy plugin install https://github.com/karlorz/llm-wiki`; root `skills/`, `agents/`, and `hooks/` are materialized mirrors of the canonical files under `packages/skills/`. A root `hooks.json` compatibility copy is also maintained, but `agy` discovers hooks from `hooks/hooks.json`.
...
Changing the layout under `packages/skills/<skill>/` requires updating `packages/skills/.claude-plugin/plugin.json`, `packages/skills/.codex-plugin/plugin.json`, running `npm run materialize:plugins` for the materialized `packages/codex-skills` layout and root `agy` layout (`skills/`, `agents/`, `hooks/`, `hooks.json`, `.claude-plugin/plugin.json`), and checking the `install` subcommand's directory scan. If the plugin root path changes, update both marketplace manifests (`.claude-plugin/marketplace.json` and `.agents/plugins/marketplace.json`).
...
- **Codex GitHub marketplace source:** `codex plugin marketplace add karlorz/llm-wiki@main` (or `--ref <branch|tag>` with Git URLs). Refresh Git-backed marketplace sources with `codex plugin marketplace upgrade llm-wiki`.
